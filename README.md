...
A modular, production-ready implementation of **Deep Markov Models** for probabilistic time series modeling. Built with PyTorch and Pyro, following SOLID design principles with full dependency injection support.

## Features

- **Probabilistic Forecasting**: Generate predictions with uncertainty quantification
- **Modular Architecture**: Swap transition, emission, and combiner networks via dependency injection
- **Type-Safe Configuration**: Dataclass-based configs with full type hints
- **Comprehensive Toolkit**: Data preprocessing, training, inference, and visualization utilities
- **Interpretability**: Extract and analyze learned latent representations
...
history = trainer.fit(train_sequences, test_sequences)

# Forecast future values
forecaster = DMMForecaster(model)
z_last = forecaster.infer_latent(observed_sequence)
predictions = forecaster.forecast(z_last, steps=24, num_samples=100)
...
```python
from dmm import preprocess_dataframe, build_sequences, train_test_split_sequences
import pandas as pd

# Load and preprocess data
df = pd.read_csv("timeseries.csv", parse_dates=["timestamp"], index_col="timestamp")
df_clean, stats = preprocess_dataframe(df, max_nan_gap=2)

# Build sequences
sequences = build_sequences(df_clean, columns=["PM10", "temp", "humidity"])
train_seqs, test_seqs = train_test_split_sequences(sequences, test_size=0.2)

# Convert to tensors
from dmm import sequences_to_tensors
train_tensors = sequences_to_tensors(train_seqs)
test_tensors = sequences_to_tensors(test_seqs)
```

### Custom Configuration

```python
from dmm import ModelConfig, TrainingConfig, DMMFactory, DMMTrainer

# Custom model architecture
model_config = ModelConfig(
    z_dim=32,              # Latent dimension
    transition_dim=128,    # Transition network hidden size
    emission_dim=64,       # Emission network hidden size
    rnn_dim=128,           # RNN hidden size
    output_dim=10,         # Number of observed variables
)
...
trainer = DMMTrainer(model, config=train_config)
```
...
The modular design allows swapping components via dependency injection:

```python
from dmm import DeepMarkovModel, BaseTransition, BaseEmitter, BaseCombiner
import torch.nn as nn
from torch import Tensor
from typing import Tuple

class CustomTransition(BaseTransition):
    """Custom transition with attention mechanism."""
    ...
    def forward(self, z_prev: Tensor) -> Tuple[Tensor, Tensor]:
        out = self.net(z_prev)
        loc = out[..., :self.z_dim]
        scale = nn.functional.softplus(out[..., self.z_dim:]) + 1e-4
        return loc, scale

# Use custom component
from dmm import Emitter, Combiner

custom_trans = CustomTransition(z_dim=16, hidden_dim=64)
emitter = Emitter(z_dim=16, emission_dim=32, output_dim=5)
combiner = Combiner(z_dim=16, rnn_dim=64)

model = DeepMarkovModel(
    transition=custom_trans,
    emitter=emitter,
    combiner=combiner,
    z_dim=16,
    rnn_dim=64,
...
```python
from dmm import DMMInterpreter

interpreter = DMMInterpreter(model)

# Extract latent trajectories
latent_traj = interpreter.extract_latent_trajectory(sequence)

# Analyze latent dynamics
dynamics_info = interpreter.analyze_dynamics(sequences)
```

### Visualization

```python
from dmm import plot_training_loss, plot_forecast

# Plot training curves
plot_training_loss(history["train_losses"], history["test_losses"])

# Plot forecast with uncertainty
plot_forecast(
    observed=test_sequence,
    mean=mean_prediction,
    lower=lower_bound,
    upper=upper_bound,
    feature_names=["PM10", "temp", "humidity"],
)
```
